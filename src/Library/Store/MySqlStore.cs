...
namespace Infrastructure.DataAccess
{
    ...
    /// </summary>
    internal class MySqlStore : IStore
    {
        /// <summary>
        /// The name of the concurrency column.
        /// </summary>
        internal static readonly string ConcurrencyColumnName = "Concurrency";
        ...
        /// <summary>
        /// Initialize an instance of MySqlStore class.
        /// </summary>
        /// <param name="factory">The connection factory.</param>
        /// <param name="databaseType">The database type to use.</param>
        internal MySqlStore(IConnectionFactory factory, DatabaseType databaseType)
        {
        ...
        protected IEnumerable<MySqlParameter> Convert(IEnumerable<IParameter> collection)
        {
            ...
            foreach (IParameter p in collection)
            {
                ...
                MySqlParameter parameter = new MySqlParameter(p.ParameterName, value) { DbType = p.DbType };
            ...
            return parameters.AsEnumerable<MySqlParameter>();
        }
            ...
            out IDictionary<string, IParameter> output)
        {
            ...
            {
                MySqlConnection connection = tx == null ? new MySqlConnection(connectionString) : tx.Connection as MySqlConnection;
                if (tx == null)
                {
                    connection.Open();
                }
                ...
                try
                {
                    using (MySqlCommand command = this.CreateSqlCommand(connection, sp, this.Convert(parameters)))
                    {
                        // Execute the command and instrument the result
                        command.Transaction = tx as MySqlTransaction;
            ...
            out IDictionary<string, IParameter> output)
        {
            ...
            if (dataSet.Tables.Count > 0 &&
                ...
                dataSet.Tables[0].Columns[0].ColumnName == ConcurrencyColumnName)
            {
                ...
                {
                    throw new InvalidDataFilterException("Concurrency check failed.");
                }
            }
            ...
            IEnumerable<MySqlParameter> parameters)
        {
            ...
            if (storedProcedure.Name.Contains(" ") == true)
            {
