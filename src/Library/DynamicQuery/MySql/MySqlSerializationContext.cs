// -----------------------------------------------------------------------
// <copyright file="MySqlSerializationContext.cs" Company="Lensgrinder, Ltd.">
...
namespace Infrastructure.DataAccess
{
    ...
    /// </summary>
    internal class MySqlSerializationContext : ISqlSerializationContext
    {
        /// <summary>
        ...
        private string SerializeCreate()
        {
            ...
            builder.AppendFormat(
                "  CREATE {2} TABLE IF NOT EXISTS {0}`{1}` (",
                schema,
        ...
        private string SerializeColumn(Column column)
        {
            ...
            if (this.ForInline == false)
            {
                ...
                builder.AppendFormat(
                    "IF (SELECT COUNT(*) FROM information_schema.columns WHERE table_schma = '{0}' AND table_name = '{1}' AND column_name = '{2}') = 0 ",
                    schema,
                    this.CurrentTable.Name,
                    column.Name);
            ...
            if (column.IsComputed == false)
            {
                builder.AppendFormat(
                    ...
                    "`{0}` {1}{2} {3} {4}{5}",
                    column.Name,
                    this.PrepareType(column),
                    this.PrepareLength(column),
                    column.IsIdentity ? this.PrepareIdentity(column) : string.Empty,
                    column.IsNullable ? "NULL" : "NOT NULL",
                    this.HandleTimestamp(column));
            }
            else
            {
                throw new NotSupportedException("Computed columns not supported.");
            }
        ...
        private string SerializeForeignKey(ForeignKey fk)
        {
            ...
            builder.AppendFormat(
                "IF (SELECT COUNT(*) FROM information_schema.constraints WHERE constraint_schema = '{0}' AND constraint_name = '{1}') = 0 ",
                fk.Owner,
                fk.Name);
        ...
        private string SerializeCommonIndex(Index ix)
        {
            ...
            builder.AppendFormat(
                CultureInfo.InvariantCulture,
                "IF (SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema = '{0}' AND table_name = '{1}' AND index_name = '{2}' AND index_schema = '{3}') = 0 ",
                this.CurrentTable.Owner,
                this.CurrentTable.Name,
                ix.Name,
                this.CurrentTable.Owner);
        ...
        private string SerializePartition(Partition partition)
        {
            ...
            {
                builder.Append(")");
                throw new NotSupportedException("Include columns not supported.");
            }
        ...
        private string SerializeSchema(Owner owner)
        {
            StringBuilder builder = new StringBuilder();
            builder.AppendLine("-----------------------------------");
            builder.AppendFormat("EXEC('CREATE SCHEMA IF NOT EXISTS `{0}`')\n", owner.Name);
            builder.Append(";");
            builder.AppendLine("-----------------------------------");
            return builder.ToString();
        }
        ...
        private string PrepareIdentity(Column column)
        {
            ...
            return string.Format(CultureInfo.InvariantCulture, "AUTO_INCREMENT = {0}", column.Seed);
        }

        /// <summary>
        /// Handle the case where the column is a rowversion.
        /// </summary>
        /// <param name="column">The column to inspect.</param>
        /// <returns>The resulting declarations.</returns>
        private string HandleTimestamp(Column column)
        {
            if (column.TypeName == "rowversion")
            {
        ...
        /// <summary>
        /// Prepares the type name.
        /// </summary>
        /// <param name="column">The column to inspect.</param>
        /// <returns>The resulting type name.</returns>
        private string PrepareType(Column column)
        {
        ...
        private string PrepareLength(Column column)
        {
            switch (column.TypeName)
            {
                ...
                case "varchar":
                case "varbinary":
                    return "(" + (column.MaxLength == -1 ? "1024" : column.MaxLength.ToString(CultureInfo.InvariantCulture)) + ")";
                case "nchar":
                case "nvarchar":
                    return "(" + (column.MaxLength == -1 ? "1024" : column.MaxLength.ToString(CultureInfo.InvariantCulture)) + ")";
                case "numeric":
                ...
                case "datetimeoffset":
                case "time":
                    return "(" + (column.Precision > 3 ? 3 : column.Precision) + ")";
                default:
                    return string.Empty;
            }
        }
