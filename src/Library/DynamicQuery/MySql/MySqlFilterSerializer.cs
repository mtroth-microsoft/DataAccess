// -----------------------------------------------------------------------
// <copyright file="MySqlFilterSerializer.cs" company="Lensgrinder, Ltd.">
...
namespace Infrastructure.DataAccess
{
    ...
    /// </summary>
    internal sealed class MySqlFilterSerializer
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of the MySqlFilterSerializer class.
        /// </summary>
        /// <param name="queryNode">The container query's composite node.</param>
        public MySqlFilterSerializer(CompositeNode queryNode)
        {
        ...
        private string SerializeAnyOrAll(
            ...
            SqlFormatter formatter)
        {
            return MySqlAnyOrAllSerializer.Serialize(anyorall, this.QueryNode, context, formatter);
        }
            ...
            ParameterContext context)
        {
            switch(name.ToLower())
            {
                ...
                case "length":
                    return string.Format("LENGTH({0})", this.SerializePredicatable(args[0], context));

                case "indexof":
                    return string.Format("INSTR({0}, {1})",
                        this.SerializePredicatable(args[0], context),
                        this.SerializePredicatable(args[1], context));
                ...
                case "hour":
                    return string.Format("HOUR({0})", this.SerializePredicatable(args[0], context));

                case "minute":
                    return string.Format("MINUTE({0})", this.SerializePredicatable(args[0], context));

                case "second":
                    return string.Format("SECOND({0})", this.SerializePredicatable(args[0], context));

                case "fractionalseconds":
                    return string.Format("MICROSECOND(ms, {0})", this.SerializePredicatable(args[0], context));

                case "totaloffsetminutes": // TODO
                    return string.Format("DATEPART(tz, {0})", this.SerializePredicatable(args[0], context));

                case "totalseconds":
                    return string.Format("TO_SECONDS({0})", this.SerializePredicatable(args[0], context));
                ...
                case "mindatetime":
                    return "CAST('001-01-01' AS datetime)";

                case "maxdatetime":
                    return "CAST('9999-12-31 23:59:59' AS datetime)";
